...
| **替换图重命名** | 在已挂载可写目录的前提下，每张替换卡可「按规范重命名」（前缀 + 主名 + 扩展名）；分组内支持「批量重命名」表。磁盘文件与 `config.json` 中的文件名会同步更新；`Svg_replace` 下全局不可重名。 |
...
**替换图文件命名（约定与机检）**：推荐小写字母 + 下划线、语义化主名、统一前缀（如 `ic_` / `img_` / `bg_`），可带 `_filled` / `_outline` 等状态后缀；界面会对非法字符、大小写、连字符与是否带推荐前缀给出提示。语义是否贴切、是否避免拼音缩写等需人工把握。
