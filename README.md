...
面向移动应用工程（如 iOS/Android 资源目录）的**图标与图片资源治理**桌面/网页工具：集中浏览素材、按语义或格式分组、配置替换图与策略，并将结果写入项目内的 `Svg_replace` 配置，便于版本管理与协作。
...
| 能力 | 说明 |
|------|------|
| **多目录浏览** | 通过清单 `assets-manifest.json` 列出分析目录，或使用「选择文件夹」即时挂载本地目录（无需预生成清单）。 |
| **资源扫描** | 递归识别 `.imageset`、常见位图（PNG/JPEG/WebP 等）、PDF、Lottie（JSON）及 SVG 等；`Svg_replace` 子树不参与素材统计。 |
| **分组与排序** | 左侧大纲 + 右侧卡片网格；支持拖拽调整分组、内置规则分组；可选接入阿里通义千问 API 做语义分组（需自行配置 Key）。 |
| **替换素材** | 按分组上传替换图（可多张），文件写入当前分析目录下的 `Svg_replace`，与 `config.json` 一并管理。 |
| **配置持久化** | 分组、替换映射等保存为分析目录内 `Svg_replace/config.json`；再次打开同一目录可自动恢复。 |
| **包体估算** | 结合替换文件大小，辅助评估变更对包体的影响。 |
| **SVG 预览改色** | 对适合单色化的线稿类 SVG，通过全局 CSS 变量统一预览 `fill`/`stroke`；多色/渐变等会自动跳过。顶栏「SVG 改色」可「随主题适配」或自定义十六进制颜色。 |
| **主题** | 亮色 / 暗色，设置保存在浏览器 `localStorage`（网页版）或同源存储策略下。 |
...
- **桌面壳**：[Tauri 2](https://v2.tauri.app/)（Rust），插件：`dialog`、`fs`（调试下可选 `log`）
...
- `src-tauri/target/release/bundle/dmg/Switch2svg_*_aarch64.dmg`（架构随本机而定）
...
脚本读取 [`switch2svg-data/analysis-folders.json`](switch2svg-data/analysis-folders.json) 中的文件夹名称列表（字段支持 `folderNames` 或 `folders` 数组），在项目根下扫描对应目录，并把结果写入 `public/assets-manifest.json`。  
...
| 形态 | 选目录方式 | 读写项目文件 |
|------|------------|--------------|
| **浏览器** | [File System Access API](https://developer.mozilla.org/en-US/docs/Web/API/File_System_Access_API)（`showDirectoryPicker`） | 依赖用户授权；需支持该 API 的浏览器 |
| **Tauri 桌面** | 原生文件夹对话框 + `tauri-plugin-fs` | 使用绝对路径读写，体验更接近本地工具 |
...
├── scripts/
│   └── build-manifest.js   # 扫描本地文件夹 → 写入 public/assets-manifest.json
├── src/                    # React 源码
│   ├── App.tsx
...
- **AI 语义分组**：API Key 与模型配置存于浏览器本地存储，仅在你主动请求分组时发往配置的模型服务端（默认文档指向阿里通义千问系列，可在界面修改模型名）。  
- **SVG 改色 / 主题**：同样使用 `localStorage` 持久化键值（见 `App.tsx` 中 `AI_CONFIG_KEY`、`THEME_KEY`、`SVG_TINT_KEY` 等）。
...
- **`app.security.csp`** 当前为 `null`，若未来加载不可信远程内容，建议按需配置 CSP。  
